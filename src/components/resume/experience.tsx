        ...
        positions: [
          {
            ...
            location: "Remote",
            desc: [
              `Automate least-privilege user management (IAM) and resource provisioning across multiple AWS accounts using AWS Organizations and AWS's Infrastructure as Code (IaC) tool, CDK.`,
              `Build secure by default CDK constructs (e.g., encryption at rest and private S3 buckets) to ease development.`,
              `Encrypt sensitive data client-side using AES-GCM envelope encryption backed by AWS KMS, random initialization vectors, and tagged ciphertexts for ease of rotation. Searchable fields leverage salted SHA-512 hashes and key stretching.`,
              `Lead Threat Model exercises determining attack vectors.`,
              `Author ERC-20 compatible Solidity smart contracts, lead external code audits, and built automated multi-blockchain deployment infrastructure.`,
              `Set up ingestion of application, CloudTrail, GuardDuty, VPC flow, and ELB access logs into Datadog. Used Cloud SIEM to alert on anomalous activity and mitigated 100% of PCI, SOC 2, and GDPR rule findings.`,
              `Planned and built cost-efficient, secure, and performant cloud infrastructure utilizing managed and serverless services and VPC endpoints.`,
              `Implemented SAML single sign-on company-wide with hardware security key multi-factor authentication (MFA) and automatic user provisioning using Google as identity provider.`,
              `Proposed and implemented multi-signature hardware wallet signing process for smart contract and cryptocurrency custodial wallet access control.`,
              `Built continuous integration and delivery (CI/CD) pipelines to ship customer features on each pull request merge.`,
              ...
              `Rolled out company-wide phishing training and engineering team OWASP Top 10 awareness.`,
