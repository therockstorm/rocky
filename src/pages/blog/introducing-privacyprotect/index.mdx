...
export const meta = {
  ...
  title:
    "Introducing PrivacyProtect: Securely Share and Store Passwords and Sensitive Files",
};
...
Dropbox and Send Files Securely aren't **end-to-end** (**E2E**) encrypted. Neither are five of the seven on Wired's list. Of the two that are, Firefox Send doesn't exist anymore, and iCloud is, but only on the newest devices and only when enabled.
...
So, when I heard about [Portable Secret](https://mprimi.github.io/portable-secret/), I smiled. A dead simple way to share and store passwords, files, and images. However, the [creator has little time for upkeep](https://github.com/mprimi/portable-secret/discussions/29), and the security landscape changes fast.

Enter [PrivacyProtect](https://www.privacyprotect.dev). Share passwords and sensitive files over email or store them in insecure locations like cloud drives using nothing more than desktop or mobile web browsers like Chrome and Safari.
...
## [Give it a try](https://www.privacyprotect.dev)
...
1. Click **Conceal secret** to generate your protected HTML file. Depending on your device's speed, it can take a few seconds.
1. Click **Save HTML file**.

<Figure alt="PrivacyProtect example." src={example} />

And just like that, your secret is protected!

**Assuming you chose a strong password**, the downloaded file is safe to share over insecure channels like email or messaging services. If the recipient cannot guess the password based on the hint, give them a call and let them know how to access it.

Here are examples of a PrivacyProtected [message](https://www.privacyprotect.dev/example-message.html) and [image file](https://www.privacyprotect.dev/example-image.html). Try "dog" for the password.

You can also safely store the HTML file in insecure locations like cloud drives (Dropbox, iCloud, Google Drive), host it like the examples above, or keep it on your computer, phone, or USB. To reveal the secret, open it in any web browser and enter the password. Even if you lose your USB drive, the secret is unreadable by anyone that finds it.

If you're short on time, you can skip the rest of this article and start using [PrivacyProtect](https://www.privacyprotect.dev) immediately; it's that simple! For those interested in details, read on.
